...
public class OgrenciEkleEkrani {

    public static VBox getPanel(Kullanici admin, Stage stage, BorderPane root) {
        Label lblBaslik = new Label("🎓 Yeni Öğrenci Ekle");
        ...
        TextField txtOgrNo   = new TextField();  txtOgrNo.setPromptText("Öğrenci No");
        ...
        btnKaydet.setOnAction(e -> {
            ...
            String no = txtOgrNo.getText().trim();
            String email = txtEmail.getText().trim();
            String sifre = txtSifre.getText().trim();
            String bolumAdi = cmbBolum.getValue();

            if (ad.isEmpty() || no.isEmpty() || email.isEmpty() || sifre.isEmpty() || bolumAdi == null) {
            ...
            try (Connection conn = DBManager.getConnection()) {
                ...
                PreparedStatement ps = conn.prepareStatement(
                    "INSERT INTO ogrenciler (ad_soyad,ogr_no,email,sifre,bolum_id) VALUES (?,?,?,?,?)");
                ps.setString(1, ad); ps.setString(2, no); ps.setString(3, email);
                ps.setString(4, sifre); ps.setInt(5, bolumId);
                ps.executeUpdate();

                show("✅ Başarılı","Öğrenci eklendi.");
                txtAdSoyad.clear(); txtOgrNo.clear(); txtEmail.clear(); txtSifre.clear(); cmbBolum.setValue(null);
        ...
        VBox form = new VBox(10, lblBaslik, txtAdSoyad, txtOgrNo, txtEmail, txtSifre, cmbBolum, btnKaydet, btnGeri);
